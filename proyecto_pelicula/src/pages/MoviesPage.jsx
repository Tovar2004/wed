...
export default function MoviesPage() {
  ...
  const fetchFn = useCallback((p) => fetchLatestMovies(p), [])
  ...
  return (
    <main className={styles.page}>
      <div className={styles.hero}>
        <h1 className={styles.heroTitle}>Películas</h1>
        <p className={styles.heroSub}>
          {data ? `${data.total.toLocaleString()} títulos disponibles` : '…'}
        </p>
      </div>

      <div className={styles.container}>
        <div className={styles.toolbar}>
          <SearchBar onSearch={handleSearch} placeholder="Buscar película por título…" isSearching={searching} />
